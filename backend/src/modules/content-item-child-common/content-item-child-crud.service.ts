...
export abstract class ContentItemChildCrudService<T extends Record<string, unknown>> extends BaseCrudService<T> {
  protected constructor(
    protected readonly childDelegate: any,
    private readonly options: ContentItemChildOptions,
    private readonly relationValidation?: RelationValidationService,
  ) {
    super(childDelegate, options.searchableFields);
  }

  async findByContentItem(contentItemId: string, query: BaseQueryDto) {
  ...
  async findChildById(contentItemId: string, id: string) {
    await this.relationValidation?.validateForeignKeys({ contentItemId });
    const item = await this.findRecord(contentItemId, id);
  ...
  async createChild(contentItemId: string, data: object, actorId: string) {
    ...
    await this.relationValidation?.validateForeignKeys({ contentItemId });
    await this.validateChildRelations(payload);
    await this.ensureUnique(contentItemId, payload);
  ...
  async updateChild(contentItemId: string, id: string, data: object, actorId: string) {
    const payload = data as Record<string, unknown>;
    await this.findRecord(contentItemId, id);
    await this.relationValidation?.validateForeignKeys({ contentItemId });
    await this.validateChildRelations(payload);
    await this.ensureUnique(contentItemId, payload, id);
  ...
  async deleteChild(contentItemId: string, id: string, actorId: string) {
    await this.findRecord(contentItemId, id);
  ...
  async restoreChild(contentItemId: string, id: string, actorId: string) {
    if (!this.hasSoftDelete()) {
      throw new NotFoundException(`${this.options.messageName} restore is not supported`);
    }
    await this.findRecord(contentItemId, id, false);
  ...
  async updateChildStatus(contentItemId: string, id: string, status: Status, actorId: string) {
    if (!this.hasStatus()) {
  ...
  protected async validateChildRelations(_data: Record<string, unknown>) {
    return undefined;
  }

  private async ensureUnique(contentItemId: string, data: Record<string, unknown>, excludeId?: string) {
    ...
    if (existing) {
      throw new ConflictException(`${this.options.messageName} already exists for this content item`);
    }
  }

  private async findRecord(contentItemId: string, id: string, excludeDeleted = true): Promise<T> {
