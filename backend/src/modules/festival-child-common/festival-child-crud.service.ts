...
export abstract class FestivalChildCrudService<T extends Record<string, unknown>> extends BaseCrudService<T> {
  protected constructor(
    protected readonly childDelegate: any,
    private readonly options: FestivalChildOptions,
    private readonly relationValidation?: RelationValidationService,
  ) {
    super(childDelegate, options.searchableFields);
  }

  async findByFestival(festivalId: string, query: BaseQueryDto) {
  ...
  async findChildById(festivalId: string, id: string) {
    await this.relationValidation?.validateForeignKeys({ festivalId });
    const item = await this.findRecord(festivalId, id);
    return createApiResponse(`${this.options.messageName} fetched successfully`, this.toResponse(item));
  }

  async createChild(festivalId: string, data: object, actorId: string) {
    const payload = data as Record<string, unknown>;
    await this.validateRelations(festivalId, payload);
    await this.ensureUnique(festivalId, payload);
  ...
  async updateChild(festivalId: string, id: string, data: object, actorId: string) {
    const payload = data as Record<string, unknown>;
    await this.findRecord(festivalId, id);
    await this.validateRelations(festivalId, payload);
    await this.ensureUnique(festivalId, payload, id);
  ...
  async deleteChild(festivalId: string, id: string, actorId: string) {
    await this.findRecord(festivalId, id);
  ...
  async restoreChild(festivalId: string, id: string, actorId: string) {
    if (!this.hasSoftDelete()) {
      throw new NotFoundException(`${this.options.messageName} restore is not supported`);
    }
    await this.findRecord(festivalId, id, false);
  ...
  async updateChildStatus(festivalId: string, id: string, status: Status, actorId: string) {
    if (!this.hasStatus()) {
  ...
  private async ensureUnique(festivalId: string, data: Record<string, unknown>, excludeId?: string) {
    ...
    if (existing) {
      throw new ConflictException(`${this.options.messageName} already exists for this festival`);
    }
  }

  private async validateRelations(festivalId: string, data: Record<string, unknown>) {
    await this.relationValidation?.validateForeignKeys({
      festivalCategoryId: this.asString(data.categoryId),
      festivalId,
  ...
  private async findRecord(festivalId: string, id: string, excludeDeleted = true): Promise<T> {
