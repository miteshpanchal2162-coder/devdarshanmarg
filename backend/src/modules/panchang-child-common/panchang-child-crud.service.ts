...
export abstract class PanchangChildCrudService<T extends Record<string, unknown>> extends BaseCrudService<T> {
  protected constructor(
    protected readonly childDelegate: any,
    private readonly options: PanchangChildOptions,
    private readonly relationValidation?: RelationValidationService,
  ) {
    super(childDelegate, options.searchableFields);
  }

  async findByPanchang(panchangId: string, query: BaseQueryDto) {
  ...
  async findChildById(panchangId: string, id: string) {
    await this.relationValidation?.validateForeignKeys({ panchangId });
    const item = await this.findRecord(panchangId, id);
  ...
  async createChild(panchangId: string, data: object, actorId: string) {
    ...
    await this.validateRelations(panchangId, payload);
    await this.ensureUnique(panchangId, payload);
  ...
  async updateChild(panchangId: string, id: string, data: object, actorId: string) {
    const payload = data as Record<string, unknown>;
    await this.findRecord(panchangId, id);
    await this.validateRelations(panchangId, payload);
    await this.ensureUnique(panchangId, payload, id);
  ...
  async deleteChild(panchangId: string, id: string, actorId: string) {
    await this.findRecord(panchangId, id);
  ...
  async restoreChild(panchangId: string, id: string, actorId: string) {
    if (!this.hasSoftDelete()) {
      throw new NotFoundException(`${this.options.messageName} restore is not supported`);
    }
    await this.findRecord(panchangId, id, false);
  ...
  async updateChildStatus(panchangId: string, id: string, status: Status, actorId: string) {
    if (!this.hasStatus()) {
  ...
  private async ensureUnique(panchangId: string, data: Record<string, unknown>, excludeId?: string) {
    ...
    if (existing) {
      throw new ConflictException(`${this.options.messageName} already exists for this panchang`);
    }
  }

  private async validateRelations(panchangId: string, data: Record<string, unknown>) {
    await this.relationValidation?.validateForeignKeys({
      cityId: this.asString(data.cityId),
      countryId: this.asString(data.countryId),
      languageId: this.asString(data.languageId),
      panchangCategoryId: this.asString(data.categoryId),
      panchangId,
      stateId: this.asString(data.stateId),
      userId: this.asString(data.userId),
    });
  }

  private async findRecord(panchangId: string, id: string, excludeDeleted = true): Promise<T> {
